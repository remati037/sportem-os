...
**Dashboard čišćenje + `computePeriodMetrics` fix + Porudžbine = SVE (promenjena odluka, potvrđeno sa korisnikom):**
- **Kartica „Porudžbine" sada broji SVE** porudžbine kreirane u periodu — **uključujući Otkazano/Vraćeno** (`brojPorudzbina = inRange.length` bez isključivanja statusa). **Zarada/promet/marža i dalje isključuju otkazane/vraćene** (nemaju stvarnu zaradu) — `computePeriodMetrics` sada radi dva brojanja: `brojPorudzbina` (svi statusi) i `realized` (bez `CANCELLED_STATUS_NAMES`) za zaradu/maržu. Neto kartica ne prikazuje broj, pa nema efekta na novčane cifre; `getNetoProfit` novčano nepromenjen.
- **Bug fix (Zarada/Marža = 0 na širokom periodu):** `.in("order_id", [1000+ UUID])` u jednom URL-u je tiho padao (`data=null`, greška se nije proveravala) → `zarada=0`, `revenue=0`, `marza=0`, dok su `brojPorudzbina`/`troskovi` radili. **Rešeno:** `order_items` upit **batchovan po 200 ID** sa `error` proverom (throw); `orders` upit **paginiran po 1000** (`.range()` loop) da broj ne podbaci na velikom opsegu.
- **Dashboard UI (`app/(app)/page.tsx`):** uklonjene kartice **„Za fakturisanje"** i **„Saldo poštarine"** (tekuće stanje); ostaje samo grid period-metrika + „Porudžbine koje čekaju" + „Niska zaliha". `getDrugMiDuguje`/`getSaldoPostarine` se više ne zovu odatle.

**Finansije bez overview stranice — default Uplate (potvrđeno sa korisnikom):**
- **`/finansije` overview (3 kartice: Drug mi duguje / Saldo poštarine / Neto profit) obrisan.** `app/(app)/finansije/page.tsx` je sada **redirect** na `/finansije/uplate` (čuva stare linkove + `revalidatePath("/finansije")`). Nav stavka „Finansije" (`lib/nav.ts`) i dalje pokazuje `/finansije` — aktivno stanje po prefiksu i dalje radi kroz sve tabove, a ulazak sleće na Uplate. `finance-tabs.tsx` nedirano. Neto profit ostaje na Dashboardu (period-metrika).

**Fakturisanje po UPLATAMA umesto porudžbina (promenjena odluka, potvrđeno sa korisnikom):**
- **„Za fakturisanje" = Σ zarade UPLATA (payouts) koje još nisu ni na jednoj fakturi** (svuda u aplikaciji). Kreiranje fakture bira **uplate**, ne pojedinačne porudžbine — jednostavnije. **Potvrđeno:** stare „plaćene" porudžbine bez uplate-reda (ručno peglane) namerno ispadaju iz „Za fakturisanje" i ne mogu se fakturisati.
- **Migracija `20260722120000_payout_invoice_link.sql`:** `payouts.invoice_id uuid REFERENCES invoices(id) ON DELETE SET NULL` + index. Model: faktura sadrži više uplata (`payouts.invoice_id`); uplata sadrži porudžbine (`orders.payout_id`). **Pre produkcije: `supabase db push`.**
- **`db/finance.ts`:** nov `getInvoiceablePayouts()` (payouts sa `invoice_id null` + Σ profit/otkup); `getInvoiceCandidates()` vraća `{ payouts: PayoutInvoiceCandidate[], total }` (bez praznih uplata); `getDrugMiDuguje()` → `{ total, payoutCount }`; `PayoutRow`/`getPayoutDetail`/`listPayouts` dobili `invoice_id`; `getBlockedNeedsVpOrders()` repurpose → needs_vp porudžbine unutar **nefakturisane** uplate (upozorenje, ne blokira). Zarada i dalje ZAMRZNUTA (`order_profit`).
- **`issueInvoice` (`actions.ts`):** prima `payout_ids` (schema izmenjena); `assertInvoiceable` proverava da su uplate `invoice_id null`; `total_amount = Σ order_profit` porudžbina tih uplata (server-side); izdavanje postavlja `payouts.invoice_id` **I kaskadno `orders.invoice_id`** (zaključava stavke; hrani netaknuti `getInvoiceDetail`/detalj fakture). `deleteInvoice` nulira i `payouts.invoice_id` i `orders.invoice_id`. Uplata na fakturi se ne može obrisati/odvezati (postojeći guard preko `orders.invoice_id`). Zaštite `placeno` + `ISTORIJA-BACKFILL` netaknute.
- **UI:** `issue-invoice-panel.tsx` bira uplate (datum · broj porudžbina · zarada); `fakture/page.tsx` banner „N nefakturisanih uplata"; lista uplata (`uplate/page.tsx`) ima badge **„Fakturisano"**. **Cron** (`api/cron/notifikacije`) podsetnik broji nefakturisane uplate („N uplata za fakturisanje drugu").
...
- **Kod:** `db/finance.ts` (`withPdv`, `getEligibleXexpressOrders`, `listXexpressInvoices`, `getXexpressInvoiceDetail`, tipovi `Xexpress*`); `lib/validation/finance.ts` (`xexpressInvoiceSchema`/`updateXexpressInvoiceSchema`); `app/(app)/finansije/actions.ts` (`createXexpressInvoice`/`updateXexpressInvoice`/`deleteXexpressInvoice`, **Admin-only**, eligibility rekompjut server-side, order update kroz `createAdminClient()` kao `updateShipping`); rute `finansije/postarina/fakture/{nova,[id],[id]/izmena}` + `xexpress-invoice-form.tsx` (create+edit) + `xexpress-invoice-actions.tsx`. Sekcija „XExpress fakture" dodata na `/finansije/postarina`. Brisanje/odvezivanje čisti `shipping_actual` nazad na null (saldo se vraća).
- **Kandidati + watermark (fix posle prve verzije):** kandidati = **Isporučeno ILI Vraćeno** (`eligibleXexpressStatusIds()`, lookup po imenu — vraćene su takođe slate pa se plaća poštarina; **Otkazano NE**, nije poslato) + `delivery_method='xexpress'` + `xexpress_invoice_id null`, **najnovije prvo**, sa pretragom po broju/kupcu. **Nema uslova da je poštarina uneta** (stare porudžbine nemaju ni naplaćenu ni stvarnu — unose se u samoj formi: forma piše **i `shipping_charged` i `shipping_actual`**, pre-popunjeno iz porudžbine). **Granica istorije (`xexpressHistoryBoundary`):** `min(ordered_at)` porudžbina koje su već na nekoj XExpress fakturi → kandidati moraju biti `ordered_at >= granica`. Dok nema nijedne fakture, granice nema (vidi se sve). **Prva faktura postavlja granicu** — sve starije od najstarije u prvoj specifikaciji trajno nestaje iz liste (rešava stare pred-app specifikacije bez ručnog rada). `assertXexpressLinkable` više NE traži `shipping_charged`.

**Popis zaliha („pogledao sam" po varijanti) + filter „Fali količina" (potvrđeno sa korisnikom):**
- **Problem:** `stock_quantity` je `not null default 0`, pa se stvarna nula („prebrojao sam, nema ništa") nije razlikovala od nikad unete količine — ceo neobrađen katalog je padao u nisko stanje i davio Dashboard listu i dnevni push.
- **Migracija `20260731120000_stock_count.sql`:** `product_variants.stock_counted_at timestamptz` + `stock_counted_by uuid → auth.users` (`on delete set null`), parcijalni index nepopisanih, i view `product_variants_public` proširen sa `stock_counted_at` (popis NIJE finansijski podatak → Logistika ga vidi). **Backfill: `stock_counted_at = updated_at` za sve `stock_quantity > 0`** (neko je tu cifru već uneo) — kao „fali količina" ostaju samo nule. **Pre produkcije: `supabase db push`** — bez toga upiti koji selektuju novu kolonu tiho vraćaju prazno (varijante nestaju iz kataloga, nisko stanje prazno), jer se PostgREST greška ne proverava.
- **Nivo = VARIJANTA** (tu stanje i živi), ne proizvod. Proizvod u listi pokazuje zbirno („Fali količina · N" kad je više varijanti).
- **Nisko stanje (promenjeno pravilo):** `isVariantLowStock` sada traži i `stock_counted_at != null`. **Popisana nula JESTE nisko stanje; nepopisana varijanta NIJE.** Isto važi za `getLowStockVariants` (Dashboard) i `lowStockCount` u cron-u (`.not("stock_counted_at","is",null)`). Dashboard ispod liste kaže koliko varijanti nema unetu količinu + link `/katalog?popis=fali` (`getUncountedVariantCount`).
- **Popis je JEDINA katalog akcija dostupna Logistici** (ona drži magacin). `setStockCount` (`katalog/actions.ts`) — `requireRole("admin","logistics")`, upis kroz **`createAdminClient()`** jer Logistika nema write RLS na `product_variants` (i dalje čita samo restriktovani view); patch dira isključivo `stock_quantity` + `stock_counted_at/by` — **nikad cene**. RLS politike iz 0.5 su NEDIRANE.
- **Kada se oznaka postavlja:** (1) ručni čekboks „Popisano" (`stock-count-control.tsx` — number input + čekboks; unos nove cifre auto-markira popis, čekboks postoji za potvrdu nepromenjenog stanja tipa 0; skidanje čekboksa ne dira količinu); (2) čuvanje forme varijante (`createVariant`/`updateVariant` stamp-uju popis — forma uvek nosi polje „Stanje"); (3) CSV uvoz **samo ako je kolona sa količinom mapirana** (`d.stock_quantity != null`).
- **UI:** katalog lista — čekboks filteri **„Stanje 0"** i **„Fali količina"** (+ `?popis=fali` iz URL-a); **popis se radi isključivo na detalju proizvoda** (`variants-table.tsx`, kolona „Stanje" postaje kontrola za Admin/Logistiku, arhivirane varijante se ne popisuju). Inline kontrola u listi je **uklonjena na zahtev korisnika** — lista ostaje čist prikaz broja + badge-ova. Badge „Fali količina" = `info` (da se ne meša sa `warning` „Nisko").
- **Filter „Stanje 0"** (odluka korisnika): proizvod prolazi ako je **bar jedna** aktivna varijanta na nuli, ali **samo POPISANA nula** se broji (`stock_quantity === 0 && stock_counted_at != null`) — nepopisana nula je „nepoznato" i pripada filteru „Fali količina", inače bi dva filtera vraćala praktično isti skup.

**Automatsko skidanje robe sa stanja po porudžbini (PROMENJENA ODLUKA — ranije „auto-decrement inventara NIJE u Fazi 1"):**
- **Problem:** stanje se menjalo isključivo ručno (forma varijante, CSV uvoz, popis) — nova porudžbina nije skidala robu, pa je `stock_quantity` bio netačan čim stigne prva porudžbina.
- **Pravilo:** roba je skinuta sa stanja dok je porudžbina u **živom toku** (Kreirano / Poslato / Isporučeno) i **vraća se** čim ode u **Otkazano ili Vraćeno**; povratak iz Otkazano/Vraćeno je **ponovo skida**. Prelazi unutar živog toka (Kreirano ↔ Poslato ↔ Isporučeno) ne diraju stanje.
- **Migracija `20260731140000_order_stock_decrement.sql`:** `orders.stock_applied boolean not null default false` (idempotentni prekidač „roba je trenutno skinuta") + RPC **`public.apply_stock_delta(p_items jsonb)`** (`security definer`, `search_path=''`, jedan atomični UPDATE nad `product_variants`; grant samo `service_role`, revoke `anon`/`authenticated`) — supabase-js ne ume `stock = stock - n` bez read-modify-write. **Pre produkcije: `supabase db push`** — bez toga svaka promena statusa/porudžbina prijavljuje „Stanje u katalogu nije ažurirano".
- **Postojeće porudžbine imaju `stock_applied = false`** (istorijski se ništa nije skidalo) → otkazivanje stare/backfill porudžbine **NEĆE** naduvati stanje. Migracija ne dira nijednu cifru stanja.
- **Stanje SME u minus** (nema clamp-a): nepopisana varijanta je 0 („količina nije uneta"), pa minus pošteno znači „prodato više nego što je evidentirano" — popis kasnije upiše tačnu cifru. Clamp na nulu bi kod vraćanja otkazane porudžbine izmišljao robu koja nikad nije skinuta (asimetrija).
- **Kod `lib/stock.ts`** (`server-only`, `createAdminClient()` interno): `syncOrderStock(orderId, "reserve"|"release")` i `syncItemStock(orderId, variantId, delta)`. Idempotentnost kroz **uslovni UPDATE** (`.eq("stock_applied", !next)`) — jedan Postgres statement, pa Woo retry ili dvostruko otkazivanje ne mogu duplo da skinu/vrate. **Best-effort, nikad ne baca** (Sentry + `false`) — isti obrazac kao `pushWooStatus`; pozivaoci dopisuju „(Stanje u katalogu nije ažurirano — proveri.)" na `success` poruku.
- **Okidači:** webhook `insertOrder` (rezerviše; **odmah-otkazana porudžbina se preskače**), webhook `syncExistingOrder` (auto-otkaz → vraća; kod **`needs_review` guarda status se ne menja pa roba ostaje skinuta** dok Admin ručno ne odluči), `changeOrderStatus` / `changeOrdersStatus` / `markCashSale` (kroz `syncStockForStatusChange`, poređenje `isCancelStatusName` staro↔novo; ime starog statusa kroz embed `status:order_statuses(name)`), i izmene stavki (`addItemFromCatalog` −kol, `deleteItem` +kol, `updateItemQuantity` razlika) — samo ako je porudžbina rezervisana.
- **Nedirano:** zamrznute cene (`order_items`), popis (`stock_counted_at` se NE stampuje automatski — promena stanja nije „pogledao sam"), RLS politike na `product_variants` (write ide kroz service-role RPC uz `requireRole` guard), finansije. Stavke bez varijante (nepoznat SKU) se preskaču.
...
- **Brisanje:** kolona `on delete restrict` → 23503 → „Kolona ima tikete — prvo ih premesti"; prioritet `set null` (tiket ostaje bez prioriteta); tag `cascade` na `ticket_tag_links` (samo se skida sa tiketa) — **preporučeno arhiviranje** (`archived_at`: ne nudi se u izboru, ostaje na starim tiketima).
- **Kod:** `lib/tickets.ts` (BEZ `server-only` — koristi se i na klijentu): `TICKET_DEFAULTS`, `TICKET_SOURCE`, `TICKET_CODE_PREFIX`/`TICKET_ARCHIVE_DAYS`/`TICKET_POSITION_STEP`, `formatTicketCode`, `parseTicketParam` (prima „SPT-42", „spt-42" i „42"). `db/tickets-config.ts` (`server-only`, RLS klijent): `getTicketColumns`, `getTicketPriorities`, `getTicketTags({includeArchived})` + `defaultPriority()`/`doneColumn()` — **zastavica, nikad hardkodovan UUID** (obrazac `APP_STATUS`). `lib/validation/tickets.ts`: zod za config CRUD. Akcije dopisane u `app/(app)/podesavanja/actions.ts` (`upsert/deleteTicketColumn|Priority|Tag`, `setTicketTagArchived`) — `requireRole("admin")` + RLS klijent, revalidate `/podesavanja` i `/tiketi`.
- **UI:** `app/(app)/podesavanja/ticket-settings.tsx` (obrazac `status-settings.tsx`) — tri sekcije (Kolone / Prioriteti / Tagovi) u novoj sekciji „Tiketi" na `/podesavanja`, **renderuje se samo Adminu**. **Board, tiketi i rute `/tiketi` NISU u T1** (dolaze u T2).

**Korak T2 — Tiketi: board, kartica, dijalog, filteri (potvrđeno sa korisnikom):**
- **Bez migracije** — cela šema je iz T1. Pristup: `requireRole("admin","manager")` na svakoj strani i akciji; **Admin i Menadžer su ravnopravni** nad tiketima, pa sve ide kroz **RLS klijent** (`*_staff_all` politike pokrivaju obe role) — nigde service-role osim čitanja imena (v. dole). Logistika dobija redirect.
- **`db/profiles.ts` (novo) čita kroz `createAdminClient()`** (jedini service-role u modulu): RLS `profiles_select` pušta korisniku samo NJEGOV red (sve ostale vidi jedino Admin), pa bi Menadžer inače dobio praznu listu izvršilaca i imena bez teksta. Vraća samo `id/full_name/role` za `admin`+`manager`; kapija je `requireRole` u pozivaocu (obrazac `/korisnici`).
- **`db/tickets.ts`:** `listTickets(filters)` → `TicketBoard` (kolone sa svojim tiketima + `total` + `archivedHidden`). Bez N+1: jedan upit tiketa, pa **batch** dopune (izvršioci, tagovi, vezane porudžbine/varijante/kupci, blokirajući tiketi) i spajanje u JS. `created_by` se **ne prosleđuje na klijent** (polja se prepisuju eksplicitno, ne spread-om). Skeniranje ograničeno na 2000 redova (bez tihog PostgREST reza). `getTicketDetail(param)` prima „SPT-42"/„42" (šifra) ili UUID (rezerva) kroz `parseTicketParam`.
- **Filteri u URL-u:** `?kolona=&osoba=&tag=&prioritet=&q=&moji=1&rok=probijen|danas&arhiva=1` (deljiv link = deljiv pogled). M:N filteri (osoba/tag) se rešavaju pred-upitom nad `ticket_assignees`/`ticket_tag_links` → presek id-jeva → `.in("id", …)`. Pretraga `q` gađa naslov (`ilike`) i šifru (`code.eq`). „Moji" gazi „osoba".
- **Arhiva:** završeni stariji od `TICKET_ARCHIVE_DAYS` (14) se **filtriraju u JS-u** posle upita da bi se izbrojali (`archivedHidden` → dugme „Prikaži arhivu"). Ništa se ne briše, nema crona.
- **Redosled i pozicija:** kartice idu `position ASC` (pa `code ASC`). Nov tiket i premešten tiket idu na **dno kolone** (`nextPositionInColumn` = `max(position) + TICKET_POSITION_STEP`). Fractional indexing između suseda je **T3** (DnD).
- **`completed_at` app NIKAD ne dira** — postavlja ga trigger iz T1 kad tiket uđe/izađe iz `is_done` kolone. Akcije šalju samo `column_id` (+ `position`).
- **Akcije `app/(app)/tiketi/actions.ts`:** `createTicket`, `updateTicket`, `deleteTicket`, `moveTicket(id, columnId)`, `setAssignees`, `setTags` + `searchTicketLinks(kind, term, excludeId)` (pretraga veza za dijalog). Izvršioci i tagovi se čuvaju kao **zamena kompletne liste** (delete + insert), a neuspeh veze ne obara upis tiketa (poruka „Tiket kreiran. (…)"). Guard: tiket ne može da čeka sam sebe.
- **Validacija (`lib/validation/tickets.ts`):** `ticketSchema`/`updateTicketSchema`/`moveTicketSchema`/`setAssigneesSchema`/`setTagsSchema`/`ticketIdSchema`. Opcione veze iz `Select`-a stižu kao **`"none"`** (Radix ne dozvoljava prazan `value`) → `null`; liste id-jeva idu kroz `formData.getAll(...)`.
- **UI:** `board.tsx` (desktop kolone, `md+`, horizontalni skrol) + `mobile-board.tsx` (tabovi po kolonama sa brojačem + lista kartica, bez DnD). **WIP limit je SOFT** — kolona broji `4/3` i pocrveni, ali ništa ne blokira. `ticket-card.tsx` (šifra, prioritet, naslov, tagovi, rok bojen po `dueState`, procena, badge-ovi veza, inicijali izvršilaca, overlay link na detalj) i `ticket-actions.tsx` („⋮": Izmeni / premesti u kolonu / Obriši).
- **`ticket-dialog.tsx`:** `TicketDialog` drži samo `open`, a sve stanje forme je u `TicketForm` **unutar `DialogContent`-a** — Radix ga demontira pri zatvaranju, pa se polja pri svakom otvaranju pune iz svežih podataka (nema zastarelog prikaza posle `router.refresh()`). Veze (porudžbina / artikal / kupac / „čeka tiket") biraju se **pretragom** (debounce 300ms) kroz `LinkPicker`; sve su opcione i nezavisne. Izbor tagova = aktivni **+ oni koji su već na tiketu** (arhiviran tag ostaje na starom tiketu, ali se ne nudi novom).
- **Nav (`lib/nav.ts`): `primary: boolean` → `primaryRoles: Role[]`.** Donji bar na telefonu: Dashboard · Porudžbine · **Tiketi** · Finansije · „Više"; **Katalog se seli u „Više" za Admina i Menadžera** (`primaryRoles: ["logistics"]`) — Logistici ostaje u baru jer im je jedini ekran. `navPrimaryForRole`/`navSecondaryForRole` filtriraju po roli; `bottom-nav.tsx` i `sidebar.tsx` nedirani.
- **Nedirano:** zamrznute cene i `order_items`, finansije, webhook tok, RLS politike drugih tabela. Veze na porudžbinu/varijantu/kupca su **samo čitanje** za prikaz.

**Detalj tiketa u MODALU nad board-om (dopuna T2, potvrđeno sa korisnikom):**
- **Odluka:** klik na karticu više ne vodi na odvojenu stranu — otvara **modal preko board-a** (ne gubi se kontekst table, Esc zatvara). **URL ostaje `/tiketi/SPT-42`**, pa deljiv link, refresh i „otvori u novom tabu" i dalje rade.
- **Kako:** Next presretnute + paralelne rute. `app/(app)/@modal/(.)tiketi/[id]/page.tsx` presreće **soft navigaciju** sa board-a i renderuje modal; direktan link / refresh / novi tab zaobilaze presretanje i dobijaju **punu stranu** `app/(app)/tiketi/[id]/page.tsx`. `app/(app)/@modal/default.tsx` vraća `null` (slot na svim ostalim rutama i posle zatvaranja). `app/(app)/layout.tsx` prima slot `modal` i renderuje ga u `AppShell` uz `children`.
- **Jedan izvor prikaza:** sadržaj je izdvojen u `app/(app)/tiketi/ticket-detail.tsx` (`TicketDetail({ param, variant })`) — strana i modal dele isti kod; `variant="modal"` izostavlja `<h1>` jer naslov nosi `DialogTitle`. Okvir modala je `ticket-modal.tsx` (klijent): lokalni `open` → `router.back()` pri zatvaranju (animacija se odigra, URL se vraća na board).
- **`getTicketDetail` je sada `cache()`-ovan** (obrazac iz `lib/auth.ts`) — modal čita naslov za zaglavlje, `TicketDetail` isti tiket za sadržaj: jedan upit po zahtevu, ne dva.
- **Brisanje iz modala** ide na `/tiketi` (`afterDeleteHref`) što ujedno gasi modal (slot pada na `default.tsx`). Edit dijalog se otvara **ugnježden** u modalu (Radix to podržava).
