...
- **UI:** katalog lista — čekboks filter „Fali količina" (+ `?popis=fali` iz URL-a) i inline popis za proizvode sa **tačno jednom** aktivnom varijantom; više varijanti → popis na detalju proizvoda (`variants-table.tsx`, kolona „Stanje" postaje kontrola za Admin/Logistiku, arhivirane varijante se ne popisuju). Badge „Fali količina" = `info` (da se ne meša sa `warning` „Nisko").
